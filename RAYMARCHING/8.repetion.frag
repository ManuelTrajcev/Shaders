...
float map(vec3 p){
    ...
    q.y -= iTime * 0.4;  //upward movement
    
    //q.xy *= rot2D(iTime*0.1);  //Exclude the axis of rotation
    
    q = fract(q) - 0.5;   // Space rpetition all axis
    //q.x = fract(p.x) - 0.5;   // Space rpetition one axis
    
    float scaleFactor = 1.0;
    
    float box = sdBox(q * scaleFactor, vec3(0.1)) / scaleFactor;    //positive scaling factor, decreases the size, divide by same amount to fix artefacts
